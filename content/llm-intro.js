  ...
  sections: [
...

    {
      ...
      body: [
        ...
        { type: "p", text: "Plug in GPT-2 small ($N{=}12$, $d{=}768$, $|V|{=}50{,}257$): the blocks give $12 \\cdot 12 \\cdot 768^2 \\approx 85$M and the embeddings $\\approx 39$M — the token matrix $|V|\\,d$ is *tied* with the unembedding, so it counts once (plus the small positional table) rather than the $2\\,|V|\\,d$ the formula assumes for untied models — totaling ~124M, the published number. The formula is worth internalizing: **it is almost all $d^2$**, which is why widening a model is so expensive and why FLOPs scale the way scaling laws say they do (later section)." },

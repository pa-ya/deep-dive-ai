...
\end{aligned}` },
...

    {
      ...
      body: [
        ...
        { type: "p", text: "Glorot & Bengio (2010) noted you want variance preserved in *both* the forward pass ($n_{\\text{in}}$ terms) and the backward pass ($n_{\\text{out}}$ terms). Since you generally can't satisfy both, take the compromise that averages the two constraints $n_{\\text{in}}\\operatorname{Var}(w)=1$ and $n_{\\text{out}}\\operatorname{Var}(w)=1$:" },
...
\end{aligned}` },
...

    {
      ...
      body: [
        ...
        { type: "p", text: "Hard one-hot targets push the network to make the correct logit $+\\infty$ and the rest $-\\infty$ — overconfident, poorly-calibrated, and prone to overfit. **Label smoothing** softens the target: mix in a uniform $\\alpha/K$ over *all* $K$ classes, so the true class ends up at $1-\\alpha+\\alpha/K$ and every other class at $\\alpha/K$:" },
